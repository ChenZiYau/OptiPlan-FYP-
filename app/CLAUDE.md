...
- **Credentials**: Stored in `app/.env` (`SUPABASE_URL` and `SUPABASE_KEY`)
- **Access**: REST API via `https://zgxmzpzuedqclfvphuqy.supabase.co/rest/v1/`
- **Auth**: Service role key (full access). Pass as `apikey` header and `Authorization: Bearer` header.
- **Tables**: `users`, `profiles`, `user_achievements`, `user_xp`, `xp_history`, `user_keys`, `user_presence`, `friendships`, `groups`, `group_members`, `group_messages`, `group_files`, `group_links`, `group_tasks`, `chat_messages`, `notebook_chat_messages`, `notebooks`, `sources`, `chunks`, `generated_notes`, `flashcards`, `quiz_questions`, `mind_maps`, `transactions`, `budgets`, `finance_settings`, `schedule_entries`, `admin_activity_log`, `app_secrets`, `dashboard_items`, `feedback`, `site_content`
