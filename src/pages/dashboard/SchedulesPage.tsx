  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-zinc-100 tracking-tight">Weekly Schedule</h2>
          <p className="text-sm text-zinc-500 mt-0.5">{schedules.length} subject{schedules.length !== 1 ? 's' : ''} scheduled</p>
        </div>
        <HoverTip label="Add a new class or event">
          <button
            onClick={() => setShowForm(true)}
            className="flex items-center justify-center sm:justify-start gap-2 px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium transition-colors duration-150 w-full sm:w-auto"
          >
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            <div className="rounded-xl bg-[#111111] border border-[#1F1F1F] p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-medium text-zinc-100">New Subject</h3>
                <button onClick={() => { resetForm(); setShowForm(false); }} className="w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 hover:text-white hover:bg-white/5 transition-colors duration-150">
                  <X className="w-4 h-4" />
                </button>
              </div>
                ...
                <div className="flex gap-2">
                  {PRESET_COLORS.map((c) => (
                    <HoverTip key={c} label="Choose a color for this entry">
                      <button
                        onClick={() => setColor(c)}
                        className={`w-7 h-7 rounded-full transition-colors duration-150 ${color === c ? 'ring-2 ring-white ring-offset-2 ring-offset-[#111111]' : ''}`}
                ...
                <div className="flex gap-1.5">
                  {DAYS.map((label, i) => (
                    <button
                      key={i}
                      onClick={() => toggleDay(i)}
                      className={`flex-1 py-2 rounded-lg text-xs font-medium transition-colors duration-150 ${
                        selectedDays.includes(i)
                          ? 'bg-indigo-600 text-white'
                          : 'bg-[#141414] text-zinc-500 hover:text-zinc-300 hover:bg-[#1A1A1A] border border-[#1F1F1F]'
                      }`}
                    >
                      {label}
                    </button>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">
                  Start Time
                  <span className="ml-2 text-indigo-400 font-normal">{formatSlot(startTime)} – {formatSlot(endTime)}</span>
                </label>
              ...
              <button
                ...
                className="w-full py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-medium transition-colors duration-150"
              >
                Add to Schedule
              </button>
            </div>
        ...
        <div className="space-y-6">
          ...
          {schedules.length > 0 && (
            <div className="rounded-xl bg-[#111111] border border-[#1F1F1F] p-4">
              <h3 className="text-sm font-medium text-zinc-100 mb-3">Subjects</h3>
                ...
                {schedules.map((s) => (
                  <div key={s.id} className="flex items-center gap-3 p-3 rounded-lg bg-white/[0.03] border border-white/5">
                    ...
                    <button
                      ...
                      className="w-7 h-7 flex items-center justify-center rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors duration-150"
                    >
          ...
          {schedules.length === 0 && (
            <div className="rounded-xl bg-[#111111] border border-[#1F1F1F] p-6 text-center">
              <p className="text-sm text-zinc-500">No subjects yet. Click "Add Subject" to create your first class.</p>
            </div>
          )}
        </div>

        {/* Timetable grid — days as rows, hours as columns */}
        <div className="rounded-xl bg-[#111111] border border-[#1F1F1F] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[800px] border-collapse">
              {/* Time header row */}
              <thead>
                <tr className="border-b border-[#1F1F1F]">
                  <th className="sticky left-0 z-10 bg-[#111111] p-3 text-xs font-semibold text-gray-500 uppercase tracking-wider text-left w-16">Day</th>
                  {HOURS.map((hour) => (
              ...
              <tbody>
                {TIMETABLE_ORDER.map((dayIndex) => (
                  <tr key={dayIndex} className="border-b border-[#1A1A1A]">
                    <td className="sticky left-0 z-10 bg-[#111111] p-3 text-xs font-semibold text-gray-400">{DAYS[dayIndex]}</td>
                    {HOURS.map((hour) => {
                          ...
                          {entries.map((entry) => {
                            ...
                            return (
                              <div
                                ...
                                className={`h-full min-h-[44px] flex items-center text-[10px] font-semibold text-white cursor-default transition-colors duration-150 ${
      ...
      {tooltip && createPortal(
        <div
          className="fixed z-[9999] pointer-events-none flex items-start gap-2.5 px-3.5 py-3 rounded-xl bg-[#1A1A1A] border border-[#2A2A2A] shadow-lg shadow-black/50 whitespace-nowrap animate-in fade-in duration-150"
          style={{ left: tooltip.x, top: tooltip.y, transform: 'translate(-50%, -100%) translateY(-8px)' }}
        >
          <div className="w-3.5 h-3.5 rounded-full shrink-0 mt-0.5" style={{ backgroundColor: tooltip.entry.color }} />
          <div className="space-y-1">
            <p className="text-sm font-medium text-zinc-100 leading-tight">{tooltip.entry.subjectName}</p>
